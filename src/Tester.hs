...
import Gradescope qualified as G
...
doTest (test, points) = do
  ...
  let correct = cases - errors - failures
      ...
      pt
        | ptRound == points && correct /= cases = ptFloor
        | otherwise = ptRound
...
createReport ps = do
  ...
  let as = map ((sum . map snd) . auto_score) auto :: [Int]
  return $
    baseReport
      { problems = ps,
...
writeScoresFile scoresFile problems =
  withFile scoresFile WriteMode $ \s -> do
    let saveAutoScore (problem, score) =
          hPutStrLn s $
...
grade s f p@Problem {..} = do
  ...
  where
    ...
    scoreSection what points =
      when (points > 0)
        . hPutStrLn f
        $ "*** " ++ what ++ " points: /" ++ show points
